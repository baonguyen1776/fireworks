 ...
 * MyMath.js
 * -----------
 * Tham chiếu toán học/lượng giác tiện dụng.
 *
...
const MyMath = (function MyMathFactory(Math) {
  ...
  // Tính khoảng cách Định lý Pythagore
  MyMath.dist = (width, height) => {
    return Math.sqrt(width * width + height * height);
  };

  // Tính khoảng cách điểm Định lý Pythagore
  // Giống như trên, nhưng lấy tọa độ thay vì kích thước.
  ...
  // Chia một vectơ tốc độ thành các thành phần x và y (góc cần phải tính bằng radian)
  MyMath.splitVector = (speed, angle) => ({
    x: Math.sin(angle) * speed,
    y: -Math.cos(angle) * speed,
  });

  // Tạo một số ngẫu nhiên giữa min (bao gồm) và max (loại trừ)
  MyMath.random = (min, max) => Math.random() * (max - min) + min;

  // Tạo một số nguyên ngẫu nhiên giữa và có thể bao gồm các giá trị min và max
  ...
  // Giới hạn một số giữa các giá trị min và max
